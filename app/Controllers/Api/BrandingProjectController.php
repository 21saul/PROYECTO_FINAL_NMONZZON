...
/*
 * =============================================================================
 * BRANDINGPROJECTCONTROLLER — CONTROLADOR HTTP (API REST, JSON)
 * =============================================================================
 * UBICACIÓN: APP/CONTROLLERS/API/BRANDINGPROJECTCONTROLLER.PHP.
...
class BrandingProjectController extends BaseApiController
{
    ...
    public function create()
    {
        ...
        if ($file && $file->isValid()) {
            ...
            $featuredPath = 'uploads/branding/' . $newName;
        }
        ...
        $data = [
            ...
            'services_provided'   => $this->request->getPost('services_provided') ?: null,
    ...
    public function update($id = null)
    {
        ...
        $update = array_filter(
            [
                ...
                'services_provided'   => $payload['services_provided'] ?? null,
    ...
    public function addImages($id = null)
    {
        ...
        $dir = FCPATH . 'uploads/branding/gallery/';
        if (! is_dir($dir) && ! mkdir($dir, 0755, true)) {
            return apiError('Could not create upload directory', 500);
        }

        $created = [];
        $last    = $this->images->where('branding_project_id', (int) $id)->orderBy('sort_order', 'DESC')->first();
        ...
        foreach ($files as $file) {
            ...
            $newName = $file->getRandomName();
            $file->move($dir, $newName);
            $url = 'uploads/branding/gallery/' . $newName;
            ++$sort;

            $imgId = $this->images->insert([
                'branding_project_id' => (int) $id,
                'image_url'           => $url,
                'alt_text'            => null,
                'sort_order'          => $sort,
            ]);
