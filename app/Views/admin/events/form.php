...
/*
 * =============================================================================
 * VISTA CI4: APP/VIEWS/ADMIN/EVENTS/FORM.PHP
 * =============================================================================
...
$event = $event ?? null;
$images = $images ?? [];
$isEdit = is_array($event) && ! empty($event['id']);
$pageTitle = $isEdit ? 'Editar evento' : 'Nuevo evento';
$this->setVar('pageTitle', $pageTitle);
$formAction = $isEdit ? base_url('admin/events/' . (int) $event['id']) : base_url('admin/events');
$currentType = old('event_type', is_array($event) ? ($event['event_type'] ?? '') : '');
$eventDateVal = old('event_date', is_array($event) ? ($event['event_date'] ?? '') : '');
if (is_string($eventDateVal) && $eventDateVal !== '' && preg_match('/^\d{4}-\d{2}-\d{2}/', $eventDateVal)) {
    $eventDateVal = substr($eventDateVal, 0, 10);
}
?>
...
<div class="admin-page-header">
    ...
    <a href="<?= base_url('admin/events') ?>" class="btn btn-admin-outline">
    ...
    <div class="card-body">
        <form action="<?= esc($formAction) ?>" method="post" enctype="multipart/form-data" class="admin-form" novalidate>
            ...
            <div class="row g-3">
                <div class="col-md-8">
                    ...
                    <input type="text" name="title" id="title" class="form-control" required maxlength="200"
                           value="<?= esc(old('title', is_array($event) ? ($event['title'] ?? '') : '')) ?>">
                </div>
                <div class="col-md-4">
                    ...
                    <input type="text" name="slug" id="slug" class="form-control" maxlength="220"
                           value="<?= esc(old('slug', is_array($event) ? ($event['slug'] ?? '') : '')) ?>">
                </div>
                <div class="col-md-6">
            ...
            <div class="mb-4 form-upload-group">
                ...
                <?php
                $feat = is_array($event) ? (string) ($event['featured_image'] ?? '') : '';
            ...
            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-admin"><?= $isEdit ? 'Guardar cambios' : 'Crear evento' ?></button>
                <a href="<?= base_url('admin/events') ?>" class="btn btn-outline-secondary">Cancelar</a>
            </div>
        </form>
    </div>
</div>
